...
class LoadXlsxToBigQuery:
    def __init__(self, bucket_name, bucket_folder, sheet_name, dataset_id, table_id, unique_key_columns):
        ...
        self.storage_client = storage.Client()  # Cliente para o Google Cloud Storage
        self.bigquery_client = bigquery.Client()  # Cliente para o BigQuery

    def load_all_excel_from_folder(self):
        """
        Carrega todos os arquivos Excel do bucket e lê a aba específica.
        """
    ...
    def ensure_table_exists(self, dataframe):
        """
        Verifica se a tabela existe no BigQuery e a cria se não existir.
        """
        table_ref = self.bigquery_client.dataset(self.dataset_id).table(self.table_id)

        try:
            self.bigquery_client.get_table(table_ref)  # Tenta obter a tabela existente
            print(f"Tabela `{self.table_id}` já existe.")
        except NotFound:
            # Criação de schema se a tabela não existe
            schema = []
    ...
    def upsert_to_bigquery(self, dataframe):
        """
        Realiza o merge (upsert) no BigQuery com base nas colunas-chave.
        """
    ...
    def process_data(self):
        """
        Combina todas as etapas para carregar os dados do GCS e realizar o upsert no BigQuery.
        """
...
# Código para testar o processamento, mas **não executar diretamente**.
